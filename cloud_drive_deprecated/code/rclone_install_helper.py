...
rclone_folder_path_mac = (
    "~/library/application support/anchorpoint software/anchorpoint/actions/rclone"
)
...
def check_winfsp_and_rclone(menu, *args, **kwargs):
    ...
    if isWin():
        winfsp_path = os.path.isfile(
            os.path.join(os.environ["ProgramFiles(x86)"], "WinFsp/bin/launcher-x64.exe")
        )
    else:
...
def show_install_dialog():
    ...
    if isWin():
        ...
        dialog.add_info(
            'When installing them you are accepting the license of <a href="https://raw.githubusercontent.com/rclone/rclone/master/COPYING">Rclone</a> and <a href="https://github.com/winfsp/winfsp/blob/master/License.txt">WinFsp</a>.'
        )
    else:
        dialog.add_text(
            "The Anchorpoint network drive is based on Rclone and FUSE-T.<br>When clicking <b>Install</b> we will download and launch the FUSE-T installer."
        )
        dialog.add_info(
            'When installing you are accepting the license of <a href="https://raw.githubusercontent.com/rclone/rclone/master/COPYING">Rclone</a>.'
        )
...
def check_and_install_winfsp():
    winfsp_path = os.path.join(
        os.environ["ProgramFiles(x86)"], "WinFsp/bin/launcher-x64.exe"
    )
    if not os.path.isfile(winfsp_path):
        ...
        winget = subprocess.run(
            "winget install -e --id WinFsp.WinFsp --accept-source-agreements",
            capture_output=True,
        )
        progress.finish()
        if winget.returncode != 0:
            print(winget.stderr)
            ui.show_error(
                "Failed to install WinFsp",
                description="Google WinFsp and install it manually.",
            )
...
def _install_rclone_async():
    ...
    if isWin():
        ...
        request_url = (
            RCLONE_INSTALL_URL_MAC_X86 if not apple_silicon else RCLONE_INSTALL_URL_MAC
        )
    ...
    if isWin():
        os.chmod(
            os.path.expanduser("~/Documents/Anchorpoint/actions/rclone/rclone.exe"),
            stat.S_IRWXU,
        )
    else:
        os.chmod(
            os.path.expanduser("~/Documents/Anchorpoint/actions/rclone/rclone"),
            stat.S_IRWXU,
        )
    ...
    dialog.add_text(
        "To finish the installation, please <b>restart Anchorpoint</b>.<br>Once that's done, you'll be able to mount your Cloud Drive."
    )
    dialog.icon = ctx.icon
    dialog.show()
