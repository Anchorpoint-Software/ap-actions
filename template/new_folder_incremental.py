    ...
    for root, _, files in os.walk(target):
        if root != target:
            ...
            if root_resolved != root: 
                aps.rename_folder(root, root_resolved)

        for file in files:
            file_resolved = file.replace("$", variable_resolved)
            if file_resolved != file: 
                aps.rename_file(os.path.join(root, file), os.path.join(root, file_resolved))

def copy_folder_async(template_folder, target_path, variable_resolved):
    try:
        aps.copy_folder(template_folder, target_path)
...
def create_folder(template_folder, target_path):
    ...
    if os.path.exists(target_path):
        ...
        ctx.run_async(copy_folder_async, template_folder, target_path, resolved)
