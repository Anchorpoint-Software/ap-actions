...
def get_workspace_template_dir(settings, fallback):
    ...
    return _get_workspace_template_dir_impl(template_dir_win, template_dir_mac, fallback)
...
dialog.add_text("Windows\t").add_input(template_dir, browse=ap.BrowseType.Folder, var="template_dir", width=400)
dialog.add_text("macOS\t").add_input(template_dir, browse=ap.BrowseType.Folder, var="template_dir_mac", width=400)
...
dialog.add_info("Use event callbacks to customize templates according to your needs. Can be a relative path.<br>For projects, place event callbacks here: <b>project/anchorpoint/templates/template_action_events.py</b>")
