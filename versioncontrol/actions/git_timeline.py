...
def parse_change(
    repo_dir: str, change, status: ap.VCFileStatus, selected: bool
) -> ap.VCPendingChange:
...
def on_load_timeline_channel_info(channel_id: str, ctx):
    ...
    try:
        ...
        ap.timeline_channel_action_processing(
            channel_id, "gitrefresh", "Refreshing Git timeline..."
        )
...
def extract_branches_from_commit_message(commit_message, current_branch):
    ...
    of_regex = (
        r"Merge branch '([\w\-\/\.]+)' of https:\/\/[\w\.\/\-]+(?: into ([\w\-\/\.]+))?"
    )
    match = re.match(of_regex, commit_message)
    if match:
        return match.group(1), match.group(2) if match.group(
            2
        ) else None if current_branch == match.group(1) else current_branch
...
def map_commit(repo, commit):
    ...
    if is_merge:
        ...
        current_branch_name = repo.get_current_branch_name()
        src_branch, target_branch = extract_branches_from_commit_message(
            commit.message, current_branch_name
        )
        if (
            target_branch == current_branch_name
            and src_branch != f"origin/{current_branch_name}"
        ):
            caption = f"Merged branch {src_branch}"
        if (
            src_branch == current_branch_name
            and target_branch
            and target_branch != f"origin/{current_branch_name}"
            and target_branch != current_branch_name
        ):
...
def on_load_timeline_channel_entries(
    ...
    try:
        ...
        if newest_committime_to_pull > 0:
            ap.set_timeline_update_count(
                ctx.project_id, channel_id, commits_to_pull, newest_committime_to_pull
            )
        else:
        ...
        if git_settings.notifications_enabled():
            ...
            if newest_commit_to_pull and last_seen_commit != newest_commit_to_pull.id:
                if commits_to_pull == 1:
                    ap.UI().show_system_notification(
                        "You have new commits",
                        "You have one new commit to pull from the server.",
                        callback=load_timeline_callback,
                    )
                else:
                    ap.UI().show_system_notification(
                        "You have new commits",
                        f"You have {commits_to_pull} new commits to pull from the server.",
                        callback=load_timeline_callback,
                    )
        ...
        workspace_settings = aps.SharedSettings(
            ctx.workspace_id, "remoteWorkspaceSettings"
        )
        if (
            git_settings.auto_lock_enabled()
            and repo.has_remote()
            and workspace_settings.get("readonlyLocksEnabled", True)
        ):
...
def on_locks_removed(locks, ctx):
    ...
    for lock in locks:
        if (
            lock.owner_id != ctx.user_id
            or "type" not in lock.metadata
            or lock.metadata["type"] != "git"
        ):
            continue
...
def handle_git_autolock(repo, ctx, changes):
    ...
    for change in changes:
        if (
            change.status != ap.VCFileStatus.New
            and change.status != ap.VCFileStatus.Unknown
            and locker.is_file_lockable(change.path)
        ):
    ...
    for lock in locks:
        if (
            lock.owner_id == ctx.user_id
            and lock.path not in all_paths
            and "type" in lock.metadata
            and lock.metadata["type"] == "git"
            and "gitbranch" not in lock.metadata
        ):
            paths_to_unlock.append(lock.path)

    ap.lock(
        ctx.workspace_id, ctx.project_id, list(paths_to_lock), metadata={"type": "git"}
    )
...
def get_cached_paths(ref, repo, changes, deleted_only=False):
    ...
    for change_path in keys:
        ...
        if (
            deleted_only
            and change.status != ap.VCFileStatus.Deleted
            and change.status != ap.VCFileStatus.Conflicted
        ):
            continue
...
def on_load_timeline_channel_pending_changes(channel_id: str, ctx):
    ...
    try:
        ...
        if not is_merging:
            parse_changes(
                repo_dir, repo.get_pending_changes(staged=True), changes, True
            )
        ...
        try:
            if auto_lock:
                handle_git_autolock(repo, ctx, info.changes)
        except Exception as e:
        ...
        if is_merging and has_conflicts:
            ...
            commit.tooltip = (
                "Cannot commit when resolving conflicts"
                if not auto_push
                else "Cannot push when resolving conflicts"
            )
        else:
            ...
            commit.tooltip = (
                ...
                if not auto_push
                else "Push your changes to Git (disable auto-push in Git settings)"
            )
...
def on_load_timeline_channel_pending_changes_async(channel_id: str, callback, ctx):
    ctx.run_async(
        run_func_wrapper,
        on_load_timeline_channel_pending_changes,
        callback,
        channel_id,
        ctx,
    )
...
def on_load_timeline_channel_entry_details(channel_id: str, entry_id: str, ctx):
    ...
    try:
        ...
        changes = dict[str, ap.VCPendingChange]()
        parse_changes(
            repo.get_root_path(), repo.get_changes_for_changelist(entry_id), changes
        )
        ...
        if not repo.commit_not_pulled(entry_id):
            ...
            revert.tooltip = (
                "Undoes all file changes from this commit. The files will show up as changed files."
                if not conflicts
                else "Cannot undo commit, resolve conflicts first"
            )
            ...
            if has_remote:
                ...
                reset.tooltip = (
                    "Resets the entire project to the state of this commit"
                    if not repo.is_push_required()
                    else "Cannot reset project, push your changes first"
                )
...
def on_load_timeline_channel_stash_details(channel_id: str, ctx):
    ...
    try:
        ...
        if not has_changes:
            ...
            apply.enabled = False
            apply.tooltip = (
                "Unable to move shelved files when you already have changed files"
            )
...
def on_load_timeline_channel_entry_details_async(
    channel_id: str, entry_id: str, callback, ctx
):
    ctx.run_async(
        run_func_wrapper,
        on_load_timeline_channel_entry_details,
        callback,
        channel_id,
        entry_id,
        ctx,
    ...
    try:
            ...
            if is_executable_running(["unrealeditor.exe"]):
                ...
                if lfsExtensions.is_extension_tracked(
                    ...
                    ap.UI().show_info(
                        ...
                        "Unreal Engine prevents the switching of branches. Please close Unreal Engine and try again",
                        duration=10000,
                    )
                    return
        ...
        try:
            ...
            if not git_errors.handle_error(e, path):
                ap.UI().show_info(
                    "Cannot switch branch",
                    "You have changes that would be overwritten, commit them first.",
                )
            return
    ...
    try:
        ...
        if repo.has_remote():
            progress = ap.Progress(
                f"Merging Branch: {branch}", show_loading_screen=True
            )
            try:
                try:
        ...
        try:
            if not repo.merge_branch(
                upstream, progress=helper.BranchProgress(progress)
            ):
    ...
    try:
        ...
        if entry_id:
            if entry_id == "vcStashedChanges":
                ...
                if not is_binary:
                    try:
                        info.modified_content = repo.get_stash_content(
                            rel_path, stash
                        ).rstrip()
                        info.original_content = repo.get_file_content(
                            rel_path, "HEAD"
                        ).rstrip()
                    except:
                        print(
                            "on_vc_get_changes_info exception: could not load stash content for changes info"
                        )
                else:
                    try:
                        # TODO: set info.original_filepath to allow an image diff view in the detail page
                        hash = repo.get_lfs_filehash(
                            paths=[rel_path], ref=f"stash@{{{stash.id}}}"
                        )[rel_path]
                ...
                if not is_binary:
                    try:
                        info.modified_content = repo.get_file_content(
                            rel_path, entry_id
                        ).rstrip()
                        info.original_content = repo.get_file_content(
                            rel_path, entry_id + "~"
                        ...
                        print(
                            "on_vc_get_changes_info exception: could not load commit content for changes info"
                        )
                else:
                    try:
                        # TODO: set info.original_filepath to allow an image diff view in the detail page
                        ...
                        if rel_path in hashes:
                            ...
                            hashes = repo.get_lfs_filehash(
                                paths=[rel_path], ref=entry_id + "^"
                            )
                            if rel_path in hashes:
                                hash = hashes[rel_path]
                        if hash:
            ...
            if not is_binary:
                try:
                    ...
                    print(
                        "on_vc_get_changes_info exception: could not load HEAD content for changes info"
                    )
