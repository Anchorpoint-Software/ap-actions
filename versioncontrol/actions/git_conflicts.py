...
def on_vc_resolve_conflicts(
    channel_id: str,
    conflict_handling: ap.VCConflictHandling,
            ...
            if conflict_handling == ap.VCConflictHandling.TakeOurs:
                ...
                if not paths:
                    progress = ap.Progress(
                        "Resolving Conflicts", show_loading_screen=True
                    )
        ...
        try:
            if repo.is_rebasing():
                ...
                if stash:
                    ...
                    dialog.add_text(
                        "Anchorpoint has created backup copies of your recent changes. They are called <b>shelved files</b>.<br>Check your project if everything is ok and clear your shelved files afterwards."
                    )
                    dialog.add_info(
                        "Learn more about <a href='https://docs.anchorpoint.app/docs/3-work-in-a-team/git/4-Resolving-conflicts/#shelved-files'>Shelved Files</a>"
                    )
...
def get_lfs_cached_file(sha256, repo_dir):
    ...
    try:
        ...
        lfs_cache_file = os.path.join(
            repo_dir, ".git", "lfs", "objects", first_digits, second_digits, sha256
        )
...
def on_vc_load_conflict_details(channel_id: str, file_path: str, ctx):
    ...
    if repo.is_merging() is False and repo.is_rebasing() is False:
        ...
        branch_incoming = (
            branch_current  # Incoming branch is pulled commit aka current branch
        )
    ...
    if branch_current:
        conflict_model.current_entry = map_commit(
            repo, repo.get_last_history_entry_for_file(rel_filepath, branch_current)
        )

    incoming_commit = map_commit(
        repo, repo.get_last_history_entry_for_file(rel_filepath, branch_incoming)
    )
    if incoming_commit:
    ...
    if lfsExtensions.is_file_tracked(file_path):
        ...
        lfs_ref_current = (
            conflict_model.current_entry.id if not is_conflict_from_stash else None
        )
        ...
        try:
            ...
            conflict_model.current_change.cached_path = (
                None
                if len(hash_current) == 0
                else get_lfs_cached_file(hash_current[rel_filepath], path)
            )
            conflict_model.incoming_change.cached_path = (
                None
                if len(hash_incoming) == 0
                else get_lfs_cached_file(hash_incoming[rel_filepath], path)
            )
        except Exception as e:
            print(f"get_lfs_filehash exception: {str(e)}")
    else:
