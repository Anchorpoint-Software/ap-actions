...
def on_timeline_detail_action(
    ...
    if action_id == "gitstashdrop":
        ...
        try:
            ...
            dialog.add_text(
                "Are you sure to <b>remove all files</b><br>in the shelf. This cannot be undone."
            )
            dialog.add_info(
                "Learn more about <a href='https://docs.anchorpoint.app/docs/version-control/features/resolving-conflicts/#shelved-files'>Shelved Files</a>"
            )
            dialog.add_button(
                "Clear", callback=lambda d: delete_button_pressed(path, channel_id, d)
    ...
    if action_id == "gitstashapply":
        ...
        try:
            ...
            if repo.has_pending_changes(True):
                ap.UI().show_info(
                    "Could not restore shelved files",
                    "You have changed files that could be overwritten. Commit them and try again",
                    duration=6000,
                )
            ...
            if not git_errors.handle_error(e, repo_path):
                if "already exists" in error:
                    ...
                    ui.show_info(
                        "Could not restore all shelved files",
                        "You have changed files that would be overwritten.  We kept the shelved files in case you need it again",
                        duration=15000,
                    )
                elif "CONFLICT" in error:
                    logging.info(
                        "Could not restore shelved files due to conflict: ", str(e)
                    )
                    ui.show_info(
                        "Shelved Files are Kept",
                        "At least one file from the shelve is conflicting. We kept the shelved files in case you need it again",
                        duration=15000,
                    )
                else:
                    raise e
