...
def delete_stash(path: str, channel_id):
    ui = ap.UI()
    try:
        progress = ap.Progress("Deleting Stash", show_loading_screen=True)
        ...
        ui.show_success("Stash successfully deleted")
        ap.close_timeline_sidebar()
        ap.refresh_timeline_channel(channel_id)
    except Exception as e:
        ui.show_error("Could not delete stash", str(e))
...
def on_pending_changes_action(channel_id: str, action_id: str, message: str, changes, all_files_selected, ctx):
    ...
    try:
        ...
        repo = GitRepository.load(path)
        if not repo: return
        progress = ap.Progress("Stashing Files", show_loading_screen=True)
        repo.stash(True)

        ui.show_success("Files stashed")
    except Exception as e:
            ui.show_error("Could not stash files", str(e))
    finally:
            return True
...
def on_timeline_detail_action(channel_id: str, action_id: str, entry_id: str, ctx: ap.Context):
    ...
    if action_id == "gitstashdrop":
        try:
            ...
            dialog = ap.Dialog()
            dialog.title = "Delete Stashed Files"
            dialog.icon = ":/icons/revert.svg"
            dialog.add_text("This will <b>delete all files</b> in your stash.<br>This cannot be undone.")
            dialog.add_empty()
