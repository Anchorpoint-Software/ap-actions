...
class GitAccountSettings(ap.AnchorpointSettings):
    def __init__(self, ctx: ap.Context):
        ...
        self.dialog.add_switch(
            True,
            var="autopush",
            text="Combine Commit and Push",
            callback=lambda d, v: refresh_timeline(d),
        )
        self.dialog.add_info(
            "Anchorpoint will automatically push your changes to the remote Git repository."
        )
        self.dialog.add_switch(
            True,
            var="autolock",
            text="Automatically lock changed files",
            callback=lambda d, v: refresh_timeline(d),
        )
        self.dialog.add_info(
            "Anchorpoint will lock all binaries that have been modified. Locks will be released when the commits are pushed to the remote Git repository. <br><a href='https://docs.anchorpoint.app/docs/general/workspaces-and-projects/file-locking/#git-projects'>Learn about File Locking</a>."
        )
        self.dialog.add_switch(
            True, var="notifications", text="Show notifications for new commits"
        )
        self.dialog.add_info(
            "Show a system notification when new commits are available on the remote Git repository."
        )

        self.dialog.add_text("Clear Git file cache automatically:\t").add_dropdown(
            "Files older than one week",
            ["Always", "Files older than one week", "Never"],
            var="autoprune",
        )
        self.dialog.add_info(
            "Clears the Git LFS cache after each push and pull to save disk space. This will never delete any data on the server or data that is not pushed to a Git remote."
        )
...
def apply_git_url(dialog, ctx, repo_path):
    ...
    if not channel:
        ap.UI().show_error(
            "Could not change URL", "The Git channel could not be loaded"
        )
        return
    ...
    try:
        ...
        if "fatal: repository" in error_message and "not found" in error_message:
            ap.UI().show_error(
                "Could not change URL",
                "The repository could not be found, did you mistype the URL?",
            )
...
def open_terminal_pressed(dialog):
    ...
    if platform.system() == "Darwin":
        ...
        if os.path.exists(ctx.project_path):
            os.system(
                f'start cmd /k "{pathlib.Path(ctx.project_path).drive} & cd {ctx.project_path}"'
            )
        else:
...
class GitProjectSettings(ap.AnchorpointSettings):
    def __init__(self, ctx: ap.Context):
        ...
        self.dialog.add_input(
            url if url is not None else "", var="url", width=400
        ).add_button(
            "Visit", var="visit", callback=open_repository_on_web, primary=False
        )
        self.dialog.add_info(
            "This changes the remote URL of your Git repository, use with caution"
        )
        self.dialog.add_button(
            "Apply URL",
            var="applyurl",
            callback=lambda d: apply_git_url_async(d, self.ctx, path),
            primary=False,
        )
        ...
        self.dialog.add_switch(
            True,
            var="gitkeep",
            text="Create .gitkeep files in new folders",
            callback=lambda d, v: d.store_settings(),
        )
        self.dialog.add_info(
            "Anchorpoint adds <i>.gitkeep</i> files to support empty folders in Git."
        )

        self.dialog.add_switch(
            True,
            var="autolfs",
            text="Automatically track all binary files as LFS files",
            callback=lambda d, v: store_shared_setting(
                "autolfs", v, self.get_shared_settings()
            ),
        )
        self.dialog.add_info(
            "Disable this to manually configure Git LFS for files using a <i>.gitattributes</i> file."
        )
        self.dialog.add_empty()

        self.dialog.add_text("<b>Auto Lock Files</b>")
        self.dialog.add_tag_input(
            ["unity"],
            "txt",
            var="lockextensions",
            width=400,
            callback=lambda d, v: store_shared_setting(
                "lockextensions", v, self.get_shared_settings()
            ),
        )
        self.dialog.add_info(
            "Anchorpoint automatically locks binary files. Add text files for automatic locking."
        )
        self.dialog.add_empty()
        ...
        self.dialog.add_button(
            "Open Git Console / Terminal", callback=open_terminal_pressed, primary=False
        )
        self.dialog.add_info(
            "Opens the Terminal / Command line with a set up Git environment.<br>Can be used to run Git commands on this computer."
        )

        self.dialog.add_button(
            "Clear Cache",
            var="prune_lfs",
            callback=lambda d: prune_pressed(d, ctx),
            primary=False,
            enabled=self.repo_available,
        )
        self.dialog.add_info(
            "Removes local files from the Git LFS cache that are old. This will never delete <br>any data on the server or data that is not pushed to a Git remote."
        )

        self.dialog.add_button(
            "Update Credentials" if repo else "Clear Credentials",
            var="updatecreds",
            callback=lambda d: update_credentials_pressed(d, ctx, path, url),
            primary=False,
        )
        if repo:
            self.dialog.add_info(
                "This will show you the login dialog again to update your credentials."
            )
        else:
            self.dialog.add_info(
                "This will clear your credentials for your configured Git remote (e.g. Azure DevOps)."
            )

        self.dialog.load_settings(self.get_settings())
        shared_settings = self.get_shared_settings()
        self.dialog.set_value(
            "lockextensions", shared_settings.get("lockextensions", ["unity"])
        )
        ...
        if self.repo_available and repo.is_sparse_checkout_enabled():
            ...
            self.dialog.add_button(
                ...
                var="reapplysparse",
                callback=lambda d: reapply_sparse_checkout_async(d, self.ctx, path),
                primary=False,
            )
            self.dialog.add_info(
                "Repairs the state of Unloaded/Downloaded folders if they don't show<br>the correct content. This may have happened after resolving conflicts."
            )
    ...
    def get_shared_settings(self):
        return aps.SharedSettings(
            self.ctx.project_id, self.ctx.workspace_id, "GitProjectSettings"
        )
