...
def check_missing_gitignore(repo, is_game_project, has_gitignore):
    if has_gitignore or not is_game_project:
        return True
    ...
    d.add_text(
        "Anchorpoint could not find a <b>.gitignore</b> file in your project.<br>Because of this, too many files would be committed"
    )
    d.add_info(
        "Add a <a href='https://docs.anchorpoint.app/docs/3-work-in-a-team/git/1-Git-basics/?highlight#gitignore'>.gitignore</a> file and commit again."
    )
    d.add_checkbox(
        callback=lambda d, v: update_gitignore_settings(d, v, path),
        var="neveraskagain",
        text="Disable this check",
    )
...
def stage_files(
    changes, all_files_selected, repo, lfs, progress, track_binary_files=True
):
...
def on_pending_changes_action(
    ...
    progress = ap.Progress(
        "Committing Files",
        "Depending on your file count and size this may take some time",
    ...
    try:
        ...
        if len(ctx.username) > 0 and len(ctx.email) > 0:
            ...
            ui.show_info(
                "Cannot set username", "Please restart Anchorpoint and try again"
            )
            return True

        repo.commit(message)
        repo.handle_sparse_checkout_after_commit(
            staged, progress=helper.SparseProgress(progress)
        )
        ...
        if auto_push and not push_in_progress(repo.get_git_dir()):
            ...
            ap.get_context().run_async(
                delay, commit_auto_push, progress, ctx, channel_id
            )
        else:
