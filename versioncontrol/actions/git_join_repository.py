...
def on_is_action_enabled(path: str, type: ap.Type, ctx: ap.Context) -> bool:
    try:
        if type != ap.Type.JoinProjectFiles:
            return False
...
if __name__ == "__main__":
        ...
        try:
            ...
            if clearCredentials:
                ...
                GitRepository.erase_credentials(
                    host, "https", path if "azure" or "visualstudio" in host else None
                )
            ...
            repo = GitRepository.clone(
                ...
                ctx.username,
                ctx.email,
                progress=helper.CloneProgress(progress),
                sparse=not download_all,
            )
            ap.evaluate_locks(workspace_id, project.id)
            progress.finish()
            helper.update_project(
                repo_path, url, join_project_files, timeline_channel, project, True
            )
            ...
            if remote_name == "Azure DevOps":
                row = row.add_button(
                    "Retry with relogin",
                    callback=lambda d: retry(clearCredentials=True),
                    primary=False,
                )
    ...
    def get_dialog_info(url: str, info_type: str):
        services = {
            "dev.azure": (
                "Azure DevOps",
                ":/icons/organizations-and-products/AzureDevOps.svg",
            ...
            "github": (
                "GitHub",
                ":/icons/organizations-and-products/github.svg",
                "GitHub",
            ),
            "gitlab": (
                "GitLab",
                ":/icons/organizations-and-products/gitlab.svg",
                "GitLab",
            ),
            "bitbucket": (
                "Bitbucket",
                ":/icons/organizations-and-products/bitbucket.svg",
                "Bitbucket",
            ),
        }
    ...
    dialog.add_info(
        "Pick an empty folder to download the project files or tell Anchorpoint where your<br> repository is located"
    )
    dialog.add_input(
        ...
        validate_callback=lambda d, v: validate_path(d, v, remote_url),
    )
    ...
    dialog.add_button(
        "Join",
        var="join",
        callback=lambda d: join_repo(d, remote_url, project, timeline_channel, ctx),
        enabled=False,
    )
