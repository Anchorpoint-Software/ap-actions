...
def file_is_binary(path: str):
    ...
    if not isinstance(mime_type[0], str):
        return _file_bytes_binary(path)
...
def lfs_track_binary_files(paths, repo, progress_callback=None):
    binary_paths, binary_extensions, canceled = _collect_binaries(
        paths, repo, progress_callback
    )
    if canceled:
        return
        ...
        with open(self.gitattributes_path, "r") as f:
            for line in f.readlines():
                ...
                if "filter=lfs" in line:
                    self.lfs_patterns.append(
                        line.split()[0].lower().replace("[[:space:]]", " ")
                    )
