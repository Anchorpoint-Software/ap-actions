...
icon_map = {
    "Godot": ":icons/organizations-and-products/godotEngine.svg",
...
def get_ignore_file_default(ignore_file_dropdown_entries, path: str):
    ...
    for ignore_file_Entry in ignore_file_dropdown_entries:
        if "Unreal" in ignore_file_Entry.name and type_exists(".uproject"):
...
if __name__ == "__main__":
    ...
    dialog.add_text("Template: ").add_dropdown(
        dropdown_values[0].name, dropdown_values, var="dropdown"
    )
    dialog.add_info(
        "Add a <b>gitignore</b> to your project to exclude certain files from being<br> committed to Git (e.g. Unreal Engine's build result)."
    )
    dialog.add_button(
        "Create", callback=lambda d: _add_git_ignore(ctx.path, ctx.yaml_dir, d)
    )
