...
def pull_async(channel_id: str, project_path):
    ...
    try:
        ...
        progress = ap.Progress(
            "Updating Git Changes", show_loading_screen=True, cancelable=True
        )

        if repo.has_pending_changes(False):
            ui.show_info(
                "Cannot pull",
                "You have to commit all your files before you can continue",
            )
            return
        ...
        if state == UpdateState.NO_REMOTE:
            ui.show_info(
                "Branch does not track a remote branch", "Push your branch first"
            )
        elif state == UpdateState.CONFLICT:
            ui.show_info(
                "Conflicts detected",
                "Please resolve your conflicts or abort the rebase",
            )
            ...
            if repo.has_pending_changes(True):
                ui.show_info(
                    "Cannot pull",
                    "You have files that would be overwritten, commit them first",
                )
            else:
