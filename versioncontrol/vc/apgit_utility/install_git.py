...
def is_git_installed():
    dir = get_git_cmddir()
    if not _check_application(get_git_cmd_path()):
        return False
    if not _check_application(get_gcm_path()):
        return False
    if not _check_application(get_lfs_path()):
        return False
    return True
...
def get_lfs_path():
    if platform.system() == "Windows":
        return os.path.join(
            get_git_cmddir(), "mingw64", "libexec", "git-core", "git-lfs.exe"
        )
    elif platform.system() == "Darwin":
...
def get_gcm_path():
    if platform.system() == "Windows":
        ...
        return os.path.join(
            get_git_cmddir(), "libexec", "git-core", "git-credential-manager-core"
        )
    else:
        raise RuntimeError("Unsupported Platform")
...
def run_git_command(args, cwd=None, **kwargs):
    ...
    try:
        p = subprocess.run(
            args, env=current_env, cwd=cwd, capture_output=True, **kwargs
        )
...
def run_git_command_with_progress(args: list, callback, cwd=None, **kwargs):
    ...
    p = subprocess.Popen(
        args, env=current_env, cwd=cwd, stdout=subprocess.PIPE, **kwargs
    )
    line_counter = 0
    while True:
...
def _setup_git():
    try:
        email = run_git_command(
            [get_git_cmd_path(), "config", "--global", "user.email"]
        )
    except:
        email = None
    try:
    ...
    if not email or email == "":
        run_git_command(
            [get_git_cmd_path(), "config", "--global", "user.email", ctx.email]
        )
    if not name or name == "":
        run_git_command(
            [get_git_cmd_path(), "config", "--global", "user.name", ctx.username]
        )
