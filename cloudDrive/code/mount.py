...
if "rclone_install_helper" in sys.modules:
    del sys.modules["rclone_install_helper"]
import rclone_install_helper as rclone_install

if "rclone_config_helper" in sys.modules:
    del sys.modules["rclone_config_helper"]
...
def setup_mount(drive, workspace_id, configuration):
    ...
    if configuration["type"] == "gcs":
        ...
        config_arguments.append(
            os.path.join(rclone_install._get_rclone_folder(), "rclone.conf")
        )
...
def setup_rclone_config(
    mount_path: str, workspace_id: str, configuration, continue_with_options: True
):
    ...
    if shared_settings.get("Config") == "":
        ...
        if password is None:
            ...
            try:
                ...
                guarantee_rclone_config_setup(
                    shared_settings.get("mount_path"),
                    workspace_id,
                    configuration,
                    first_setup=True,
                )
...
def guarantee_rclone_config_setup(
    mount_path: str, workspace_id: str, configuration, first_setup: bool
) -> bool:
    if configuration["type"] != "gcs":
        if first_setup:
            show_options(mount_path, workspace_id, configuration)
    ...
    if not os.path.isfile(conf_file_path):
        ctx.run_async(
            setup_rclone_config, mount_path, workspace_id, configuration, first_setup
        )
        return False
...
def on_application_started(ctx: ap.Context):
    try:
        ...
        try:
            resolve_configuration(shared_settings, configuration, password)
            if not guarantee_rclone_config_setup(
                drive, ctx.workspace_id, configuration, first_setup=False
            ):
                return
        except:
            return
        if check_internet_connection():
            ...
            ui = ap.UI()
            ui.show_error(
                "Cannot mount the Cloud Drive", "You are not connected to the Internet"
            )
    except:
        pass
