...
def on_add_user_to_workspace(email, ctx: ap.Context):
    client = GitlabClient(
        ctx.workspace_id,
        ap.get_config().gitlab_client_id,
        ap.get_config().gitlab_client_key,
    )
    ...
    try:
        ...
        ap.UI().show_error(
            title="Cannot add member to GitLab",
            duration=10000,
            description=f'Cannot to add the member to the group. You have to add your member <a href="https://gitlab.com/groups/{current_group.path}/-/group_members">directly on GitLab</a>.',
        )
...
def on_remove_user_from_workspace(email, ctx: ap.Context):
    ...
    try:
        ...
        ap.UI().show_error(
            ...
            duration=10000,
            description=f'Failed to remove member from group. You have to remove your member <a href="https://gitlab.com/groups/{current_group.path}/-/group_members">directly on GitLab</a>.',
        )
    ...
    try:
        ...
        dialog.add_button(
            ...
            callback=lambda d: open_browser_and_close_dialog(
                d,
                f"https://gitlab.com/{current_group.path}/{repo_name}/-/project_members",
            ),
        )
        dialog.show()
...
def on_remove_user_from_project(email, ctx: ap.Context):
    ...
    try:
        ...
        ap.UI().show_error(
            title="Cannot remove member from GitLab project",
            duration=10000,
            description=f'Failed to remove member, because "{str(e)}". You have to remove your member <a href="https://gitlab.com/{current_group.path}/{repo_name}/-/project_members">directly on GitLab</a>.',
        )
        return
...
def show_test_repo_error_dialog(client: GitlabClient, message):
    ...
    dialog.add_button(
        ...
        callback=retry_credential_setup,
        primary=False,
    )
    dialog.show()
...
def create_test_repo_async(client: GitlabClient):
    ...
    ap.UI().show_success(
        title="GitLab Integration Test sucessful",
        duration=3000,
...
class GitlabIntegration(ap.ApIntegration):
    def __init__(self, ctx: ap.Context):
        ...
        self.client = GitlabClient(
            ctx.workspace_id, config.gitlab_client_id, config.gitlab_client_key
        )
    ...
    def show_settings_dialog(self, current_group, groups):
        ...
        dialog.add_image(
            os.path.join(self.ctx.yaml_dir, "gitlab/gitLabCredentials.webp"), width=230
        )
        dialog.add_info(
        ...
        try:
            ...
            if "has already been taken" in str(e):
                ...
                ap.UI().show_error(
                    title="Cannot create GitLab Repository",
                    duration=8000,
                    description=f'Failed to create, because "{str(e)}". Please try again<br>or check our <a href="https://docs.anchorpoint.app/docs/general/integrations/gitlab">troubleshooting</a>.',
                )
            raise e
