...
class GitlabClient:
    ...
    def start_auth(self):
        ...
        authorization_url, _ = oauth.authorization_url(
            gitlab_auth_url, state=self.state
        )
    ...
    def _get_user_groups(self):
        ...
        response = self.oauth.get(
            f"{gitlab_api_url}/groups?min_access_level={min_access_level}"
        )
        if not response:
    ...
    def create_project(self, group: Group, name: str):
        ...
        if not response:
            ...
            raise Exception(
                f"Could not create repository: {response.text} and status code {response.status_code}"
            )
    ...
    def _get_project_id_by_name(self, project_name, group: Group):
        projects_url = (
            f"{gitlab_api_url}/projects?search={project_name}&membership=true"
        )
    ...
    def _remove_invited_user_from_project(self, project_id, user_email):
        remove_invite_url = (
            f"{gitlab_api_url}/projects/{project_id}/invitations/{user_email}"
        )
