...
def on_add_user_to_workspace(email, ctx: ap.Context):
    ...
    if current_org is None:
        ap.UI().show_error(
            title="Cannot add member to Gitea",
            duration=6000,
            description="Failed to get current organization. You have to add your member directly on Gitea.",
        )
        return
    ...
    if not client.init():
        ap.UI().show_error(
            title="Cannot add member to Gitea",
            duration=6000,
            description="Failed to connect integration. You have to add your member directly on Gitea.",
        )
        return
    ...
    try:
        ...
        ap.UI().show_success(
            title="Member added to Gitea",
            duration=3000,
            description=f"User {email} added to organization {current_org.name}.",
        )
        ...
        ap.UI().show_error(
            title="Cannot add member to Gitea",
            duration=10000,
            description=f'Cannot to add the member to the organization. You have to add your member <a href="{client.get_host_url()}/org/{current_org.name}/teams/owners">directly on Gitea</a>.',
        )
...
def on_remove_user_from_workspace(email, ctx: ap.Context):
    ...
    if current_org is None:
        ap.UI().show_error(
            title="Cannot remove member to Gitea",
            duration=6000,
            description="Cannot get current organization. You have to remove your member directly on Gitea.",
        )
        return
    ...
    if not client.init():
        ap.UI().show_error(
            title="Cannot remove member to Gitea",
            duration=6000,
            description="Failed to connect integration. You have to remove your member directly on Gitea.",
        )
        return
    ...
    try:
        ...
        ap.UI().show_success(
            title="Member removed from Gitea",
            duration=3000,
            description=f"User {email} removed from organization {current_org.name}.",
        )
    except Exception as e:
        print(str(e))
        ap.UI().show_error(
            title="Cannot remove member from Gitea",
            duration=10000,
            description=f'Failed to remove member from organization. You have to remove your member <a href="{client.get_host_url()}/org/{current_org.name}/teams/owners">directly on Gitea</a>.',
        )
...
def on_add_user_to_project(email, ctx: ap.Context):
    ...
    supports_all_tags = all(
        tag in project_integration_tags.split(";") for tag in integration_tags
    )
    ...
    if project is None:
        ap.UI().show_error(
            title="Cannot add member to Gitea repository",
            duration=6000,
            description=f"Failed to find project with id {ctx.projectId}. Please add manually.",
        )
        return
    ...
    if not client.is_setup():
        ap.UI().show_error(
            title="Cannot add member to Gitea repository",
            duration=6000,
            description="Gitea integration is not setup. Please add manually.",
        )
        return
    ...
    if not client.init():
        ap.UI().show_error(
            title="Cannot add member to Gitea repository",
            duration=6000,
            description="Failed to connect integration. Please add manually.",
        )
        return
    ...
    try:
        ...
        ap.UI().show_success(
            title="Member added to Gitea repository",
            duration=3000,
            description=f"User {email} added to repository {project.name}.",
        )
    except Exception:
        ...
        dialog.add_button(
            "Add Member on Gitea",
            callback=lambda d: open_browser_and_close_dialog(
                d,
                f"{client.get_host_url()}/{current_org.name}/{repo_name}/settings/collaboration",
            ),
        )
        dialog.show()
        return
...
def on_remove_user_from_project(email, ctx: ap.Context):
    ...
    if project is None:
        ap.UI().show_error(
            title="Cannot remove member from Gitea repository",
            duration=6000,
            description=f"Failed to find project with id {ctx.projectId}. Please add manually.",
        )
        return
    ...
    try:
        ...
        ap.UI().show_success(
            title="Member removed from Gitea repository",
            duration=3000,
            description=f"User {email} removed from project {project.name}.",
        )
    except Exception as e:
        repo_name = client.generate_gitea_repo_name(project.name)
        ap.UI().show_error(
            title="Cannot remove member from Gitea repository",
            duration=10000,
            description=f'Failed to remove member, because "{str(e)}". You have to remove your member <a href="{client.get_host_url()}/{current_org.name}/{repo_name}/settings/collaboration">directly on Gitea</a>.',
        )
        return
...
def setup_credentials_async(dialog, host_url: str):
    ...
    try:
        ...
        if (
            result is None
            or result.get("host") is None
            or result["host"] != netloc
            or result.get("username") is None
            or result.get("password") is None
        ):
            raise Exception("Login failed")
        GitRepository.store_credentials(
            netloc, scheme, result["username"], result["password"], path
        )
        ap.UI().show_success(
            title="Gitea credentials stored",
            duration=3000,
            description="Gitea credentials stored successfully.",
        )
    except Exception as e:
        if "User cancelled dialog" in str(e):
            ap.UI().show_error(
                title="Cannot store Gitea credentials",
                duration=6000,
                description="Failed to store credentials, because you cancelled the credential dialog. Please try again.",
            )
        else:
            ap.UI().show_error(
                title="Cannot store Gitea credentials",
                duration=6000,
                description=f'Failed to store credentials, because "{str(e)}". Please try again.',
            )
    finally:
...
def show_test_repo_error_dialog(client: GiteaClient, message):
    ...
    dialog.add_button(
        "Retry", callback=lambda d: retry_create_test_repo(client, d), primary=True
    ).add_button(
        "Update Credentials",
        var=settings_credential_btn_entry,
        callback=lambda d: retry_credential_setup(d, client.get_host_url()),
        primary=False,
    )
    dialog.show()
...
def create_test_repo_async(client: GiteaClient):
    ...
    try:
        progress = ap.Progress(
            "Testing Gitea Integration",
            "Creating Anchorpoint-Test repository",
            infinite=True,
        ...
        if "401" in str(e):
            show_test_repo_error_dialog(
                client, get_dialog_create_message("your are not authorized")
            )
        elif "403" in str(e):
            show_test_repo_error_dialog(
                client,
                get_dialog_create_message(
                    f"you do not have permission to create a repository in {current_org.name}"
                ),
            )
        else:
            ap.log_error(f"Gitea - Create Test Project Error: {str(e)}")
            show_test_repo_error_dialog(
                client, get_dialog_create_message("of an unknown error")
            )
        progress.finish()
        return
    ...
    try:
        ...
        if "fatal: repository" in message and "not found" in message:
            show_test_repo_error_dialog(
                client,
                get_dialog_clone_message(
                    "you do not have permission to clone the repository"
                ),
            )
        else:
            ap.log_error(f"Gitea - Unknown Clone Test Repo Error: {message}")
            show_test_repo_error_dialog(
                client, get_dialog_clone_message("of an unknown error")
            )
        return
    finally:
    ...
    ap.UI().show_success(
        title="Gitea Integration Test sucessful",
        duration=3000,
        description='Test repository "Anchorpoint-Test" created and cloned successfully.',
    )
...
class GiteaIntegration(ap.ApIntegration):
    ...
    def execute_preferences_action(self, action_id: str):
        if action_id == connect_action_id:
            ...
            try:
                ...
                ap.UI().show_error(
                    title="Cannot load Gitea Settings",
                    duration=6000,
                    description=f'Failed to load, because "{str(e)}". Please try again.',
                )
                return
    ...
    def on_auth_deeplink_received(self, url: str):
        try:
            ...
            ap.UI().show_error(
                ...
                duration=6000,
                description=f'The authentication failed, because "{str(e)}". Please try again.',
            )
            return
    ...
    def setup_create_project_dialog_entries(self, action_id, dialog: ap.Dialog):
        if action_id == create_repo_dialog_entry:
            if self.is_setup:
                dialog.add_info(
                    "You may need to <b>log into</b> Gitea again after the final step.",
                    var=create_dialog_info_entry,
                )
                return [create_dialog_info_entry]
            return []

    def on_create_project_dialog_entry_selected(
        self, action_id: str, dialog: ap.Dialog
    ):
        # stub
        return
    ...
    def validate_url(self, dialog: ap.Dialog, value: str):
        ...
        if url_pattern.match(value) is None:
            dialog.set_value(
                client_values_info_entry, "Please insert your valid Gitea url first."
            )
            return "Please insert a valid url"
    ...
    def connect_to_server(self, dialog: ap.Dialog):
        ...
        if reachable:
            self.client.store_for_workspace(
                host_url=server_url, client_id=client_id, client_secret=client_secret
            )
            self.client.setup_workspace_settings()
            ap.UI().show_success(
                title="Connected to Gitea",
                duration=3000,
                description="You are now connected to Gitea.<br>Please continue with the authentication.",
            )
            self.client.start_auth()
            self.start_auth()
            dialog.close()
            ...
            ap.UI().show_error(
                title="Cannot connect to Gitea",
                duration=6000,
                description="Failed to connect to Gitea.<br>Please check your url and try again.",
            )
    ...
    def show_workspace_setup_dialog(self):
        ...
        dialog.add_input(
            placeholder="https://mygiteaserver.com:3030",
            var=server_url_entry,
            width=400,
            validate_callback=self.validate_url,
        )

        dialog.add_text("<b>2. Gitea OAuth Application</b>", var="oauthapp")
        dialog.add_info(
            "Insert your valid Gitea url first", var=client_values_info_entry
        )
        ...
        dialog.add_input(
            ...
            var=client_id_entry,
            width=400,
            validate_callback=self.validate_client_id,
        )
        dialog.add_text("Client Secret")
        dialog.add_info("This key must have 56 characters")
        dialog.add_input(
            placeholder="gto_876s8df768768768769sfg68f76g8...",
            var=client_secret_entry,
            width=400,
            validate_callback=self.validate_client_secret,
        )

        dialog.add_button(
            "Connect to Gitea",
            var=connect_to_server_btn_entry,
            callback=lambda d: self.connect_to_server(d),
            enabled=False,
        )
    ...
    def show_settings_dialog(self, current_org, orgs):
        ...
        dialog.add_dropdown(
            current_org.name,
            dropdown_entries,
            var=settings_org_dropdown_entry,
            callback=lambda d, v: self.change_org_callback(d, v, orgs),
        )
        dialog.add_info(
            "Allow Anchorpoint to create repositories and add<br>members in a organization."
        )
        dialog.add_empty()

        dialog.add_text("<b>3. Git Credentials</b>")
        dialog.add_image(
            os.path.join(self.ctx.yaml_dir, "gitea/giteaCredentials.webp"), width=230
        )
        dialog.add_info(
            "Opens the Git Credential Manager, where you need to<br>enter your Gitea login data to grant Anchorpoint<br>permission to upload and download files."
        )
        dialog.add_button(
            "Enter your Gitea credentials",
            var=settings_credential_btn_highlight_entry,
            callback=self.credential_btn_callback,
        )
        dialog.add_button(
            "Enter your Gitea credentials",
            var=settings_credential_btn_entry,
        ...
        dialog.add_info(
            'Anchorpoint will create and clone a repository called<br>"Anchorpoint-Test" to check if the integration is working<br>properly. You can delete this repository later.'
        )
        dialog.add_button(
            "Create Test Repository",
            callback=lambda d: self.create_test_repo_btn_callback(d),
        )
        dialog.add_empty()
    ...
    def show_clear_integration_dialog(self):
        ...
        dialog.add_text(
            "Do you also want to remove the gitea server infos (url,<br>client id and client secret) for all workspace members?"
        )
        dialog.add_checkbox(
            text="Delete gitea server infos from workspace",
            var=remove_data_entry,
            default=False,
        )

        dialog.add_button(
            "Disconnect", var="disconnect", callback=self.clear_integration
        )
        dialog.show()
    ...
    def create_new_repo(
        self, project_id: str, project_name: str, progress: ap.Progress
    ) -> str:
        current_org = self.client.get_current_organization()
        try:
            ...
            settings = aps.SharedSettings(
                project_id, self.ctx.workspace_id, "integration_info"
            )
            ...
            if "already exists" in str(e):
                ap.UI().show_error(
                    title="Cannot create Gitea Repository",
                    duration=8000,
                ...
                ap.UI().show_error(
                    title="Cannot create Gitea Repository",
                    duration=8000,
                    description=f'Failed to create, because "{str(e)}". Please try again<br>or check our <a href="https://docs.anchorpoint.app/docs/general/integrations/gitea">troubleshooting</a>.',
                )
            raise e
