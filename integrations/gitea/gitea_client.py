...
class GiteaClient:
    ...
    def start_auth(self):
        ...
        authorization_url, _ = oauth.authorization_url(
            f"{self.host_url}/login/oauth/authorize", state=self.state, verify=False
        )
    ...
    def oauth2_response(self, response_url: str):
        ...
        token = self.oauth.fetch_token(
            f"{self.host_url}/login/oauth/access_token",
    ...
    def store_for_workspace(self, host_url: str, client_id: str, client_secret: str):
        sharedSettings = aps.SharedSettings(
            self.workspace_id, gitea_shared_settings_key
        )
        host_url = host_url.rstrip("/")
        sharedSettings.set(
            gitea_host_url_key, base64.b64encode(host_url.encode()).decode()
        )
        sharedSettings.set(
            gitea_client_id_key, base64.b64encode(client_id.encode()).decode()
        )
        sharedSettings.set(
            gitea_client_secret_key, base64.b64encode(client_secret.encode()).decode()
        )
    ...
    def _get_current_user(self):
        ...
        return Organization(
            ...
            if data["login_name"] is not None and data["login_name"] != ""
            else data["login"],
        ...
        if not response:
            if "already exists" in response.text:
                return self.create_project(
                    org, self._get_auto_adjusted_project_name(name)
                )
    ...
    def add_user_to_organization(self, org: Organization, user_email: str):
        ...
        for member in members:
            if member.get("email") == user_email or self._email_matches_username(
                user_email, member.get("login")
            ):
                return
    ...
    def remove_user_from_repository(
        self, org: Organization, user_email: str, name: str
    ):
        owner = org.name
        repo = self.generate_gitea_repo_name(name)
        username = self._get_username_from_email(user_email)
